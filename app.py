        ...
        with gr.Tabs() :
            with gr.Tab("Main") :
                gr.Markdown("""
                    ...
                    1. 정량적 지표 기반 평가 결과물이 아닌 **실제 사용자들이 체감하는 만족도 기반의 새로운 평가 지표를 만들자.** 
                ...
                gr.Markdown("""
                    ...
                    ✅ **모델 평가 기준**
                    - **정보 정확도**  
                    - **문장 완성도** : 문장의 끝맺음 처리 능력
                    - **문해력** : 문장을 이해하고 필요한 부분만 추출해 처리하는 능력
                    - **논리적 근거** : 정답에 대한 뒷받침이 되는 설명을 추출하는 능력
                    ...
                    - **F1 SCORE** : Precision(정밀도)과 Recall(재현율)의 조화 평균으로 계산  
                      □ 모델이 정답으로 출력한 부분에서 실제로 맞은 정답 비율  
                      □ 실제 정답 중 모델이 놓친 비율  
                    - **TF-IDF** : 요약 능력 평가  
                ...
                with gr.Blocks() as final :
                    ...
                    gr.Markdown("""
                        ## 결론
                        위와 같은 평가를 통해 미세조정, RAG 탑재, 파인튜닝, 하이퍼 파라미터 수정 등으로 **Granite v3** 모델의 성능을 15 ~ 20% 정도로 향상 시킨다면면 
