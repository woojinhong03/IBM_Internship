...
def build_app() :
    with gr.Blocks(css="""
        ...
        with gr.Tabs() :
            ...
            시중에는 이미 다양한 정량적 지표가 존재하지만, 실제로 우리가 직접 사용해본 결과, **정량적 지표의 순위와는 다른 성능을 보이는 경우가 많았습니다.**  
            이에 따라, 단순히 점수로 나타난 지표만이 아니라, **실제 사용자들이 체감하는 만족도**를 기반으로 최적의 모델을 선정하는 데 도움을 주고자 하였습니다.  
            또한, 우리가 직접 평가한 결과를 참고하는 것뿐만 아니라, 사용자가 직접 모델을 체험하고 평가함으로써 **자신에게 가장 적합한 모델**을 찾을 수 있도록 돕는 것이 목표입니다.
