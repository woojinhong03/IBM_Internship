...
def build_app() :
    with gr.Blocks(css="""
        ...
        with gr.Tabs() :
            with gr.Tab("Main") :
                gr.Markdown("""
                    # <span style="color:#AAAAAA">2025 WINTER IBM P-TECH Internship</span>  
                    # 📊 LLM 성능 비교 프로젝트 📊  
                    <br/>
                      
                      
                    &nbsp; ✅ 이 프로젝트는 **LLM의 성능을 비교**하는 것이 주 목적이며, 특히 **한국어 사용 성능**을 비교&분석하는 것을 목표로 합니다.  
                      
                    &nbsp; 시중에는 이미 LLM을 비교하는 다양한 지표가 존재합니다.   
                    하지만 저희가 일상 생활 속에서 여러 LLM들을 직접 사용해본 결과, **지표(리더보드)의 순위와 다른 성능을 보이는 경우가 많았습니다.**  
                    이 부분에 의문을 가지고 이유를 분석해 보니 대표적으로 두 가지의 이유가 있었습니다.
                    
                    1. **정량적 결과물**만을 활용한 지표가 대부분이다.
                    2. **영어 기반 평가**이기 때문에 한국어 사용시의 성능은 고려되지 않았다.
                    
                    &nbsp; 이에 따라, 저희는 **"새로운 리더보드를 만들어보자"** 라는 결론에 도달했습니다.  
                    기준은 다음과 같습니다.  
                ...
                with gr.Blocks() as Image_of :
                    ...
                    with gr.Row() :
                        gr.Image("img/정량적평가(KeyModel).png", label = "(1) Comparison 8b")
                        gr.Image("img/상위모델포함 정량평가.png", label = "(2) Top Model")

                    gr.Markdown("""
                        🔑 **(1) Comparison 8b**  
                        &nbsp; 정량적 평가는 주어진 정답에서 한 글자라도 틀리다면 틀린 것으로 간주하기 때문에 전반적으로 낮은 점수를 나타냅니다.  
                        **Key Model**인 **Granite v3** 모델과 비교해 큰 차이가 확인되지 않지만, **TF-IDF** 지표에서 **Gemini 1.5 flash** 모델보다 점수가 10% 정도 높게 나오는 것을 알 수 있습니다.  
                        이유는 **TF-IDF** 평가 방식이 지문에서 많은 문장을 인용할수록 높은 점수를 주는 방식이기 때문입니다.  
                        또한 **LLAMA v3.1** 모델의 경우 **TF-IDF** 점수가 타 모델에 비해 유독 높게 나오는데  
                        앞서 위에서 설명한 **TF-IDF** 방식이 지문에서 가장 많은 문장을 인용한 부분에서 점수를 주기 때문입니다.
                        해당 지표를 통해 **Granite v3**와 **Gemini 1.5 flash** 모델의 성능 차이가 약 9% 정도 나오는 것을 확인할 수 있습니다.  
                        Gemini 모델의 수치가 높게 나오는 이유는 해당 모델이 배포 되기 전 단계에서 파인튜닝이 되어있는 상태이기 때문인 것으로 추측 할 수 있습니다.
                                    
                        📈 **(2) Top Model**  
                        **Top Model** 지표를 참고하면, Top 모델인 **Mixtral 8x7 v1**의 성능과 **Granite v3**와 약 10% 정도의 성능 차이가 나는 것을 확인할 수 있습니다.  
                        또한  **Gemini 1.5 flash** 모델이 Top 모델보다 **Rouge-1과 Rouge-L** 평가 부분에서 약 3% 정도 높다는 것을 확인할 수 있습니다.  
                        그 외의 사항으론 Top 모델이 **Gemini 1.5 flash** 5% 정도 앞서고 있다는 것을 확인할 수 있습니다. 
                ...
                with gr.Blocks() as Image:
                    ...
                    gr.Markdown("""
                        📊 **(1) Total Score**  
                        **Total Score 지표**의 경우 각 정성 평가의 항목 점수를 모두 더한 값으로 **Gemini 1.5 flash** 모델이 가장 높은 수치가 나온 것을 보여줍니다.  
                        파인튜닝 된 **Gemini 1.5 flash**를 제외하면 **Granite v3** 모델 뒤 **LLAMA** 모델 순으로 성능이 좋다는 것을 확인 할 수 있습니다.
                            
                        🔑 **(2) Key Model**  
                        IBM LLM과 타 사 LLM 성능을 비교하기 위해 **Granite v3** 모델을 Key Model로 선정하였습니다.  
                        위 지표를 통해 **Total Score**와 비슷한 내용으로 **Gemini 1.5 flash** 모델이 사용자에게 기본 모델보다 10% 정도 좋으며, **Granite**와 **LLAMA**가 비등비등한 모습을 보여줍니다.  
                                    
                        📈 **(3) Top Model**  
                        파라미터의 수가 더 많은 모델인 **Mixtral 8x7 v1**과 비교 했을 때, 파인튜닝 된 모델인 **Gemini 1.5 flash**이 약 560개의 파라미터 개수를 가진  **Mixtral 8x7 v1** 모델과 성능이 비슷한 것을 확인 할 수 있습니다.  
